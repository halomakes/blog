...
var e = "generated", t = "pointerdown", n = "pointerup", r = "pointerleave", i = "pointerout", a = "pointermove", o = "touchstart", s = "touchend", c = "touchmove", l = "touchcancel", u = "resize", d = "visibilitychange", f = "tsParticles - Error", p = .5, m = 1e3, h = {
	x: 0,
	y: 0,
	z: 0
}, g = {
	...
	c: 0,
	d: 1
}, _ = "random", v = Math.PI * 2, y = "true", b = "false", x = "canvas", S = .25, ee = p + S, te = -S, ne = 1.5, C;
(function(e) {
...
var ie = class e {
	constructor(e, t, n) {
		if (this._updateFromAngle = (e, t) => {
			...
			let t = e;
			this.z = t.z ? t.z : h.z;
		} else if (e !== void 0 && t !== void 0) this.x = e, this.y = t, this.z = n ?? h.z;
		else throw Error(`${f} Vector3d not initialized correctly`);
	}
	static get origin() {
		return e.create(h.x, h.y, h.z);
	}
	get angle() {
	...
	rotate(t) {
		return e.create(this.x * Math.cos(t) - this.y * Math.sin(t), this.x * Math.sin(t) + this.y * Math.cos(t), h.z);
	}
	setTo(e) {
		this.x = e.x, this.y = e.y;
		let t = e;
...
}, k = class e extends ie {
	constructor(e, t) {
		super(e, t, h.z);
	}
	static get origin() {
		return e.create(h.x, h.y);
	}
	static clone(t) {
...
function A() {
	return j(ae(), 0, 1 - 2 ** -52);
}
function se(e) {
	return oe.nextFrame(e);
}
function ce(e) {
	oe.cancel(e);
}
...
function P(e, t) {
	if (e === t || t === void 0 && T(e)) return e;
	let n = ue(e), r = de(e);
	return t === void 0 ? P(n, r) : {
		min: Math.min(n, t),
		max: Math.max(r, t)
	};
}
...
function pe(e, t, n) {
	if (T(e)) return fe(e);
	switch (e) {
		case C.top: return -Math.PI * p;
		case C.topRight: return -Math.PI * S;
		case C.right: return 0;
		case C.bottomRight: return Math.PI * S;
		case C.bottom: return Math.PI * p;
		case C.bottomLeft: return Math.PI * ee;
		case C.left: return Math.PI;
		case C.topLeft: return -Math.PI * ee;
		case C.inside: return Math.atan2(n.y - t.y, n.x - t.x);
		case C.outside: return Math.atan2(t.y - n.y, t.x - n.x);
		default: return A() * v;
	}
}
function me(e) {
...
function he(e) {
	return {
		x: e.position?.x ?? A() * e.size.width,
		y: e.position?.y ?? A() * e.size.height
	};
}
function ge(e) {
...
function Te() {
	return !we() && typeof matchMedia < "u";
}
function Ee(e) {
	if (Te()) return matchMedia(e);
}
function De(e) {
	if (!(we() || typeof IntersectionObserver > "u")) return new IntersectionObserver(e);
}
function Oe(e) {
	if (!(we() || typeof MutationObserver > "u")) return new MutationObserver(e);
}
function ke(e, t) {
	return e === t || D(t) && t.indexOf(e) > -1;
}
function Ae(e) {
	return Math.floor(A() * e.length);
}
function je(e, t, n = !0) {
	return e[t !== void 0 && n ? t % e.length : Ae(e)];
}
function Me(e, t, n, r, i) {
	return Ne(Pe(e, r ?? 0), t, n, i);
}
function Ne(e, t, n, r) {
	let i = !0;
	return (!r || r === R.bottom) && (i = e.top < t.height + n.x), i && (!r || r === R.left) && (i = e.right > n.x), i && (!r || r === R.right) && (i = e.left < t.width + n.y), i && (!r || r === R.top) && (i = e.bottom > n.y), i;
}
function Pe(e, t) {
	return {
		bottom: e.y + t,
...
function z(e, ...t) {
	for (let n of t) {
		...
		t && (E(e) || !e || !Array.isArray(e)) ? e = [] : !t && (E(e) || !e || Array.isArray(e)) && (e = {});
		for (let t in n) {
			if (t === "__proto__") continue;
			let r = n[t], i = e;
			i[t] = E(r) && Array.isArray(r) ? r.map((e) => z(i[t], e)) : z(i[t], r);
		}
	}
	return e;
}
function Fe(e, t) {
	return D(e) ? e.map((e, n) => t(e, n)) : t(e, 0);
}
function B(e, t, n) {
	return D(e) ? je(e, t, n) : e;
}
function Ie(e, t) {
	let n = e.value, r = e.animation, i = {
		delayTime: N(r.delay) * m,
		enable: r.enable,
		value: N(e.value) * t,
		max: de(n) * t,
		min: ue(n) * t,
	...
	if (r.enable) {
		switch (i.decay = 1 - N(r.decay), r.mode) {
			case _e.increase:
				i.status = L.increasing;
				break;
			case _e.decrease:
				i.status = L.decreasing;
				break;
			case _e.random:
				i.status = A() >= .5 ? L.increasing : L.decreasing;
				break;
		}
		let e = r.mode === _e.auto;
		switch (r.startValue) {
			case be.min:
				i.value = i.min, e && (i.status = L.increasing);
				break;
			case be.max:
				i.value = i.max, e && (i.status = L.decreasing);
				break;
			case be.random:
			default:
				i.value = M(i), e && (i.status = A() >= .5 ? L.increasing : L.decreasing);
				break;
		}
	}
...
function Be(e, t, n, r, i) {
	...
	if (t.time ||= 0, (t.delayTime ?? 0) > 0 && t.time < (t.delayTime ?? 0) && (t.time += i.value), !((t.delayTime ?? 0) > 0 && t.time < (t.delayTime ?? 0))) {
		switch (t.status) {
			case L.increasing:
				t.value >= s ? (n ? t.status = L.decreasing : t.value -= s, t.loops ||= 0, t.loops++) : t.value += a;
				break;
			case L.decreasing: t.value <= o ? (n ? t.status = L.increasing : t.value += s, t.loops ||= 0, t.loops++) : t.value -= a;
		}
		t.velocity && c !== 1 && (t.velocity *= c), ze(e, r, t.value, o, s), e.destroyed || (t.value = j(t.value, o, s));
	}
}
function Ve(e) {
	...
	for (let n in e) {
		let r = e[n];
		if (!Object.prototype.hasOwnProperty.call(e, n) || O(r)) continue;
...
function V(e, t, n, r = !0) {
	if (!t) return;
	let i = w(t) ? { value: t } : t;
	if (w(i.value)) return Ke(e, i.value, n, r);
	if (D(i.value)) return V(e, { value: je(i.value, n, r) });
...
function Ke(e, t, n, r = !0) {
	if (!t) return;
	let i = w(t) ? { value: t } : t;
	if (w(i.value)) return i.value === "random" ? Ze() : Ye(e, i.value);
	if (D(i.value)) return Ke(e, { value: je(i.value, n, r) });
	for (let t of e.colorManagers.values()) {
...
function Ze(e) {
	let t = e ?? 0;
	return {
		b: Math.floor(M(P(t, 256))),
		g: Math.floor(M(P(t, 256))),
		r: Math.floor(M(P(t, 256)))
	};
}
function U(e, t) {
...
function $e(e, t, n, r) {
	let i = e, a = t;
	return i.r === void 0 && (i = H(e)), a.r === void 0 && (a = H(t)), {
		b: le(i.b, a.b, n, r),
		g: le(i.g, a.g, n, r),
		r: le(i.r, a.r, n, r)
	};
...
function tt(e, t, n, r) {
	let i = w(t) ? t : t.value;
	return i === "random" ? r ? V(e, { value: i }) : n ? _ : "mid" : i === "mid" ? "mid" : V(e, { value: i });
}
function nt(e) {
...
function rt(e, t, n) {
	...
	return t && (it(r.h, t.h, n), it(r.s, t.s, n), it(r.l, t.l, n)), r;
}
function it(e, t, n) {
	e.enable = t.enable, e.enable ? (e.velocity = N(t.speed) / 100 * n, e.decay = 1 - N(t.decay), e.status = L.increasing, e.loops = 0, e.maxLoops = N(t.count), e.time = 0, e.delayTime = N(t.delay) * m, t.sync || (e.velocity *= A(), e.value *= A()), e.initialValue = e.value, e.offset = P(t.offset)) : e.velocity = 0;
}
function at(e, t, n, r) {
	...
	let i = e.offset ? M(e.offset) : 0, a = (e.velocity ?? 0) * r.factor + i * 3.6, o = e.decay ?? 1, s = de(t), c = ue(t);
	!n || e.status === L.increasing ? (e.value += a, e.value > s && (e.loops ||= 0, e.loops++, n ? e.status = L.decreasing : e.value -= s)) : (e.value -= a, e.value < 0 && (e.loops ||= 0, e.loops++, e.status = L.increasing)), e.velocity && o !== 1 && (e.velocity *= o), e.value = j(e.value, c, s);
}
function ot(e, t) {
	if (!e) return;
	let { h: n, s: r, l: i } = e, a = {
	...
	n && at(n, a.h, !1, t), r && at(r, a.s, !0, t), i && at(i, a.l, !0, t);
}
//#endregion
...
function ct(e, t, n) {
	e.fillStyle = n ?? "rgba(0,0,0,0)", e.fillRect(h.x, h.y, t.width, t.height);
}
function lt(e, t, n, r) {
	n && (e.globalAlpha = r, e.drawImage(n, h.x, h.y, t.width, t.height), e.globalAlpha = 1);
}
function ut(e, t) {
	e.clearRect(h.x, h.y, t.width, t.height);
}
function dt(e) {
	let { container: t, context: n, particle: r, delta: i, colorStyles: a, backgroundMask: o, composite: s, radius: c, opacity: l, shadow: u, transform: d } = e, f = r.getPosition(), p = r.rotation + (r.pathRotation ? r.velocity.angle : 0), m = {
		...
		a: m.cos * (d.a ?? g.a),
		b: h ? m.sin * (d.b ?? 1) : d.b ?? g.b,
		c: h ? -m.sin * (d.c ?? 1) : d.c ?? g.c,
		d: m.cos * (d.d ?? g.d)
	};
	n.setTransform(_.a, _.b, _.c, _.d, f.x, f.y), o && (n.globalCompositeOperation = s);
	let v = r.shadowColor;
	u.enable && v && (n.shadowBlur = u.blur, n.shadowColor = U(v), n.shadowOffsetX = u.offset.x, n.shadowOffsetY = u.offset.y), a.fill && (n.fillStyle = a.fill);
	let y = r.strokeWidth ?? 0;
	n.lineWidth = y, a.stroke && (n.strokeStyle = a.stroke);
	let b = {
		container: t,
		context: n,
		particle: r,
	...
	pt(b), mt(b), ft(b), n.globalCompositeOperation = "source-over", n.resetTransform();
}
function ft(e) {
...
var bt = class {
	constructor(e, t) {
		this.container = e, this._applyPostDrawUpdaters = (e) => {
			...
			let t, n;
			for (let r of this._colorPlugins) if (!t && r.particleFillColor && (t = qe(this._engine, r.particleFillColor(e))), !n && r.particleStrokeColor && (n = qe(this._engine, r.particleStrokeColor(e))), t && n) break;
			return [t, n];
		}, this._initCover = async () => {
			let e = this.container.actualOptions.backgroundMask.cover, t = e.color;
			if (t) {
				let n = V(this._engine, t);
				if (n) {
					let t = {
						...n,
						a: e.opacity
					};
					this._coverColorStyle = U(t, t.a);
				}
			...
			this.draw((t) => ct(t, this.size, e));
		}, this._paintImage = (e, t) => {
			this.draw((n) => lt(n, this.size, e, t));
		}, this._repairStyle = () => {
			let e = this.element;
			if (!e) return;
			...
			let e = this.element;
			e && yt(e, Ue(this.container.actualOptions.fullScreen.zIndex), !0);
	...
	clear() {
		...
		e.backgroundMask.enable ? this.paint() : t.enable && t.length > 0 && n ? n.color ? this._paintBase(U(n.color, n.opacity)) : n.image && this._paintImage(n.image, n.opacity) : e.clear && this.draw((e) => {
			ut(e, this.size);
		});
	}
	destroy() {
	...
	drawParticle(e, t) {
		...
		a ||= r, o ||= i, !(!a && !o) && this.draw((r) => {
			let i = this.container, s = i.actualOptions, c = e.options.zIndex, l = 1 - e.zIndexFactor, u = l ** c.opacityRate, d = e.bubble.opacity ?? e.opacity?.value ?? 1, f = e.strokeOpacity ?? d, p = d * u, m = f * u, h = {}, g = { fill: a ? Qe(a, p) : void 0 };
			g.stroke = o ? Qe(o, m) : g.fill, this._applyPreDrawUpdaters(r, e, n, p, g, h), dt({
				container: i,
				context: r,
				particle: e,
				delta: t,
	...
	drawParticlePlugin(e, t, n) {
		this.draw((r) => gt(r, e, t, n));
	}
	drawPlugin(e, t) {
		this.draw((n) => ht(n, e, t));
	}
	async init() {
		this._safeMutationObserver((e) => e.disconnect()), this._mutationObserver = Oe((e) => {
	...
	initBackground() {
		...
		if (n) {
			if (e.color) {
				let t = V(this._engine, e.color);
				n.backgroundColor = t ? U(t, e.opacity) : "";
	...
	loadCanvas(t) {
		this._generated && this.element && this.element.remove(), this._generated = t.dataset && "generated" in t.dataset ? t.dataset[e] === "true" : this._generated, this.element = t, this.element.ariaHidden = "true", this._originalStyle = Ve(this.element.style);
		let n = this._standardSize;
		n.height = t.offsetHeight, n.width = t.offsetWidth;
		let r = this.container.retina.pixelRatio, i = this.size;
		t.height = i.height = n.height * r, t.width = i.width = n.width * r, this._context = this.element.getContext("2d"), this._safeMutationObserver((e) => e.disconnect()), this.container.retina.init(), this.initBackground(), this._safeMutationObserver((e) => {
			!this.element || !(this.element instanceof Node) || e.observe(this.element, { attributes: !0 });
		});
	}
	paint() {
		...
		this.draw((t) => {
			e.backgroundMask.enable && e.backgroundMask.cover ? (ut(t, this.size), this._coverImage ? this._paintImage(this._coverImage.image, this._coverImage.opacity) : this._coverColorStyle ? this._paintBase(this._coverColorStyle) : this._paintBase()) : this._paintBase();
		});
	}
	resize() {
	...
	stop() {
		this._safeMutationObserver((e) => e.disconnect()), this._mutationObserver = void 0, this.draw((e) => ut(e, this.size));
	}
	async windowResize() {
...
function W(e, t, n, r, i) {
	if (r) {
		let r = { passive: !0 };
		re(i) ? r.capture = i : i !== void 0 && (r = i), e.addEventListener(t, n, r);
	} else {
		let r = i;
	...
	constructor(e) {
		this.container = e, this._doMouseTouchClick = (e) => {
			...
			if (this._canPush) {
				...
				Fe(i.mode, (e) => this.container.handleClickMode(e));
			}
			...
			this._resizeTimeout = setTimeout(() => void e(), this.container.actualOptions.interactivity.events.resize.delay * m);
		}, this._manageInteractivityListeners = (e, r) => {
			let i = this._handlers, u = this.container, d = u.actualOptions, f = u.interactivity.element;
			if (!f) return;
			let p = f, m = u.canvas;
			m.setPointerEvents(p === m.element ? "initial" : "none"), (d.interactivity.events.onHover.enable || d.interactivity.events.onClick.enable) && (W(f, a, i.mouseMove, r), W(f, o, i.touchStart, r), W(f, c, i.touchMove, r), d.interactivity.events.onClick.enable ? (W(f, s, i.touchEndClick, r), W(f, n, i.mouseUp, r), W(f, t, i.mouseDown, r)) : W(f, s, i.touchEnd, r), W(f, e, i.mouseLeave, r), W(f, l, i.touchCancel, r));
		}, this._manageListeners = (e) => {
			let t = this._handlers, n = this.container, a = n.actualOptions.interactivity.detectsOn, o = n.canvas.element, s = r;
			a === xt.window ? (n.interactivity.element = window, s = i) : a === xt.parent && o ? n.interactivity.element = o.parentElement ?? o.parentNode : n.interactivity.element = o, this._manageMediaMatch(e), this._manageResize(e), this._manageInteractivityListeners(s, e), document && W(document, d, t.visibilityChange, e, !1);
		}, this._manageMediaMatch = (e) => {
			let t = this._handlers, n = Ee("(prefers-color-scheme: dark)");
			if (n) {
				if (n.addEventListener !== void 0) {
					W(n, "change", t.themeChange, e);
					return;
				}
			...
			if (!e) return;
			let t = e.mouse;
			delete t.position, delete t.clickPosition, delete t.downPosition, e.status = r, t.inside = !1, t.clicking = !1;
			...
			if (e.type.startsWith("pointer")) {
				...
				} else if (n.interactivity.detectsOn === xt.parent) {
					let e = t.target, n = t.currentTarget;
					if (e && n && i) {
						let r = e.getBoundingClientRect(), a = n.getBoundingClientRect(), s = i.getBoundingClientRect();
						o = {
							x: t.offsetX + 2 * r.left - (a.left + s.left),
							y: t.offsetY + 2 * r.top - (a.top + s.top)
						};
					} else o = {
			...
			let s = t.retina.pixelRatio;
			o && (o.x *= s, o.y *= s), r.mouse.position = o, r.status = a;
		}, this._touchEnd = (e) => {
...
var K = class e {
	...
	static create(t, n) {
		let r = new e();
		return r.load(t), n !== void 0 && (w(n) || D(n) ? r.load({ value: n }) : r.load(n)), r;
	}
	load(e) {
		O(e) || O(e.value) || (this.value = e.value);
	}
}, Ct = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.color !== void 0 && (this.color = K.create(this.color, e.color)), e.image !== void 0 && (this.image = e.image), e.position !== void 0 && (this.position = e.position), e.repeat !== void 0 && (this.repeat = e.repeat), e.size !== void 0 && (this.size = e.size), e.opacity !== void 0 && (this.opacity = e.opacity));
	}
}, wt = class {
	constructor() {
		this.opacity = 1;
	...
	load(e) {
		O(e) || (e.color !== void 0 && (this.color = K.create(this.color, e.color)), e.image !== void 0 && (this.image = e.image), e.opacity !== void 0 && (this.opacity = e.opacity));
	}
}, Tt = class {
	constructor() {
		this.composite = "destination-out", this.cover = new wt(), this.enable = !1;
	}
	load(e) {
		if (!O(e)) {
			if (e.composite !== void 0 && (this.composite = e.composite), e.cover !== void 0) {
				let t = e.cover, n = w(e.cover) ? { color: e.cover } : e.cover;
	...
	load(e) {
		O(e) || (e.enable !== void 0 && (this.enable = e.enable), e.zIndex !== void 0 && (this.zIndex = e.zIndex));
	}
}, Dt = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.enable !== void 0 && (this.enable = e.enable), e.mode !== void 0 && (this.mode = e.mode));
	}
}, Ot;
(function(e) {
...
var kt = class {
	constructor() {
		this.selectors = [], this.enable = !1, this.mode = [], this.type = Ot.circle;
	}
	load(e) {
		O(e) || (e.selectors !== void 0 && (this.selectors = e.selectors), e.enable !== void 0 && (this.enable = e.enable), e.mode !== void 0 && (this.mode = e.mode), e.type !== void 0 && (this.type = e.type));
	}
}, At = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.enable !== void 0 && (this.enable = e.enable), e.force !== void 0 && (this.force = e.force), e.smooth !== void 0 && (this.smooth = e.smooth));
	}
}, jt = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.enable !== void 0 && (this.enable = e.enable), e.mode !== void 0 && (this.mode = e.mode), this.parallax.load(e.parallax));
	}
}, Mt = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.delay !== void 0 && (this.delay = e.delay), e.enable !== void 0 && (this.enable = e.enable));
	}
}, Nt = class {
	constructor() {
		this.onClick = new Dt(), this.onDiv = new kt(), this.onHover = new jt(), this.resize = new Mt();
	}
	load(e) {
		if (O(e)) return;
		this.onClick.load(e.onClick);
		let t = e.onDiv;
		t !== void 0 && (this.onDiv = Fe(t, (e) => {
			let t = new kt();
			return t.load(e), t;
	...
	load(e) {
		O(e) || (e.position && (this.position = {
			x: e.position.x ?? 50,
			y: e.position.y ?? 50,
			mode: e.position.mode ?? ye.percent
		}), e.options && (this.options = z({}, e.options)));
	}
}, Lt;
(function(e) {
...
var Rt = class {
	constructor() {
		this.maxWidth = Infinity, this.options = {}, this.mode = Lt.canvas;
	}
	load(e) {
		O(e) || (O(e.maxWidth) || (this.maxWidth = e.maxWidth), O(e.mode) || (e.mode === Lt.screen ? this.mode = Lt.screen : this.mode = Lt.canvas), O(e.options) || (this.options = z({}, e.options)));
	}
}, q;
(function(e) {
...
var zt = class {
	constructor() {
		this.auto = !1, this.mode = q.any, this.value = !1;
	}
	load(e) {
		O(e) || (e.auto !== void 0 && (this.auto = e.auto), e.mode !== void 0 && (this.mode = e.mode), e.value !== void 0 && (this.value = e.value));
	}
}, Bt = class {
	constructor() {
		this.name = "", this.default = new zt();
	}
	load(e) {
		O(e) || (e.name !== void 0 && (this.name = e.name), this.default.load(e.default), e.options !== void 0 && (this.options = z({}, e.options)));
	}
}, Vt = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.count !== void 0 && (this.count = P(e.count)), e.enable !== void 0 && (this.enable = e.enable), e.speed !== void 0 && (this.speed = P(e.speed)), e.decay !== void 0 && (this.decay = P(e.decay)), e.delay !== void 0 && (this.delay = P(e.delay)), e.sync !== void 0 && (this.sync = e.sync));
	}
}, Ht = class extends Vt {
	constructor() {
		super(), this.mode = _e.auto, this.startValue = be.random;
	}
	load(e) {
		super.load(e), !O(e) && (e.mode !== void 0 && (this.mode = e.mode), e.startValue !== void 0 && (this.startValue = e.startValue));
	}
}, Ut = class extends Vt {
	constructor() {
	...
	load(e) {
		super.load(e), !O(e) && e.offset !== void 0 && (this.offset = P(e.offset));
	}
}, Wt = class {
	constructor() {
		this.h = new Ut(), this.s = new Ut(), this.l = new Ut();
	}
	load(e) {
		O(e) || (this.h.load(e.h), this.s.load(e.s), this.l.load(e.l));
	}
}, Gt = class e extends K {
	constructor() {
		super(), this.animation = new Wt();
	}
	static create(t, n) {
		let r = new e();
		return r.load(t), n !== void 0 && (w(n) || D(n) ? r.load({ value: n }) : r.load(n)), r;
...
var qt = class {
	...
	load(e) {
		O(e) || e.speed !== void 0 && (this.speed = e.speed);
	}
}, Jt = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.enable !== void 0 && (this.enable = e.enable), e.retries !== void 0 && (this.retries = e.retries));
	}
}, Yt = class {
	constructor() {
		this.value = 0;
	...
	load(e) {
		O(e) || O(e.value) || (this.value = P(e.value));
	}
}, Xt = class extends Yt {
	constructor() {
	...
	constructor() {
		this.horizontal = new Qt(), this.vertical = new Qt();
	}
	load(e) {
		O(e) || (this.horizontal.load(e.horizontal), this.vertical.load(e.vertical));
	}
}, en = class {
	constructor() {
		this.absorb = new qt(), this.bounce = new $t(), this.enable = !1, this.maxSpeed = 50, this.mode = Kt.bounce, this.overlap = new Jt();
	}
	load(e) {
		O(e) || (this.absorb.load(e.absorb), this.bounce.load(e.bounce), e.enable !== void 0 && (this.enable = e.enable), e.maxSpeed !== void 0 && (this.maxSpeed = P(e.maxSpeed)), e.mode !== void 0 && (this.mode = e.mode), this.overlap.load(e.overlap));
	}
}, tn = class {
	constructor() {
	...
	load(e) {
		...
		if (t !== void 0) for (let e in t) {
			let n = t[e];
			n && (this.options[e] = z(this.options[e] ?? {}, n));
		}
	...
	load(e) {
		O(e) || (e.offset !== void 0 && (this.offset = P(e.offset)), e.value !== void 0 && (this.value = P(e.value)));
	}
}, rn = class {
	constructor() {
	...
	load(e) {
		if (!O(e) && (e.distance !== void 0 && (this.distance = P(e.distance)), e.enable !== void 0 && (this.enable = e.enable), e.rotate)) {
			let t = e.rotate.x;
	...
	constructor() {
		this.x = 50, this.y = 50, this.mode = ye.percent, this.radius = 0;
	}
	load(e) {
		O(e) || (e.x !== void 0 && (this.x = e.x), e.y !== void 0 && (this.y = e.y), e.mode !== void 0 && (this.mode = e.mode), e.radius !== void 0 && (this.radius = e.radius));
	}
}, on = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.acceleration !== void 0 && (this.acceleration = P(e.acceleration)), e.enable !== void 0 && (this.enable = e.enable), e.inverse !== void 0 && (this.inverse = e.inverse), e.maxSpeed !== void 0 && (this.maxSpeed = P(e.maxSpeed)));
	}
}, sn = class {
	constructor() {
		this.clamp = !0, this.delay = new Yt(), this.enable = !1, this.options = {};
	}
	load(e) {
		O(e) || (e.clamp !== void 0 && (this.clamp = e.clamp), this.delay.load(e.delay), e.enable !== void 0 && (this.enable = e.enable), this.generator = e.generator, e.options && (this.options = z(this.options, e.options)));
	}
}, cn = class {
	load(e) {
		O(e) || (e.color !== void 0 && (this.color = K.create(this.color, e.color)), e.image !== void 0 && (this.image = e.image));
	}
}, ln = class {
	constructor() {
		this.enable = !1, this.length = 10, this.fill = new cn();
	}
	load(e) {
		O(e) || (e.enable !== void 0 && (this.enable = e.enable), e.fill !== void 0 && this.fill.load(e.fill), e.length !== void 0 && (this.length = e.length));
	}
}, J;
(function(e) {
...
var un = class {
	...
	load(e) {
		O(e) || (e.default !== void 0 && (this.default = e.default), this.bottom = e.bottom ?? e.default, this.left = e.left ?? e.default, this.right = e.right ?? e.default, this.top = e.top ?? e.default);
	}
}, dn = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.acceleration !== void 0 && (this.acceleration = P(e.acceleration)), e.enable !== void 0 && (this.enable = e.enable), e.position && (this.position = z({}, e.position)));
	}
}, fn = class {
	constructor() {
		this.angle = new nn(), this.attract = new rn(), this.center = new an(), this.decay = 0, this.distance = {}, this.direction = C.none, this.drift = 0, this.enable = !1, this.gravity = new on(), this.path = new sn(), this.outModes = new un(), this.random = !1, this.size = !1, this.speed = 2, this.spin = new dn(), this.straight = !1, this.trail = new ln(), this.vibrate = !1, this.warp = !1;
	}
	load(e) {
		if (O(e)) return;
		this.angle.load(T(e.angle) ? { value: e.angle } : e.angle), this.attract.load(e.attract), this.center.load(e.center), e.decay !== void 0 && (this.decay = P(e.decay)), e.direction !== void 0 && (this.direction = e.direction), e.distance !== void 0 && (this.distance = T(e.distance) ? {
		...
		let t = e.outModes;
		t !== void 0 && (E(t) ? this.outModes.load(t) : this.outModes.load({ default: t })), this.path.load(e.path), e.random !== void 0 && (this.random = e.random), e.size !== void 0 && (this.size = e.size), e.speed !== void 0 && (this.speed = P(e.speed)), this.spin.load(e.spin), e.straight !== void 0 && (this.straight = e.straight), this.trail.load(e.trail), e.vibrate !== void 0 && (this.vibrate = e.vibrate), e.warp !== void 0 && (this.warp = e.warp);
	}
}, pn = class extends Ht {
	constructor() {
		super(), this.destroy = ve.none, this.speed = 2;
	}
	load(e) {
		super.load(e), !O(e) && e.destroy !== void 0 && (this.destroy = e.destroy);
	}
}, mn = class extends Zt {
	constructor() {
		super(), this.animation = new pn(), this.value = 1;
	}
	load(e) {
		if (O(e)) return;
		super.load(e);
...
var _n = class {
	constructor() {
		this.mode = gn.delete, this.value = 0;
	}
	load(e) {
		O(e) || (e.mode !== void 0 && (this.mode = e.mode), e.value !== void 0 && (this.value = e.value));
	}
}, vn = class {
	constructor() {
		this.density = new hn(), this.limit = new _n(), this.value = 0;
	}
	load(e) {
		O(e) || (this.density.load(e.density), this.limit.load(e.limit), e.value !== void 0 && (this.value = e.value));
	}
}, yn = class {
	constructor() {
		this.blur = 0, this.color = new K(), this.enable = !1, this.offset = {
			x: 0,
			y: 0
		}, this.color.value = "#000";
	}
	load(e) {
		O(e) || (e.blur !== void 0 && (this.blur = e.blur), this.color = K.create(this.color, e.color), e.enable !== void 0 && (this.enable = e.enable), e.offset !== void 0 && (e.offset.x !== void 0 && (this.offset.x = e.offset.x), e.offset.y !== void 0 && (this.offset.y = e.offset.y)));
	}
}, bn = class {
	constructor() {
	...
	constructor() {
		super(), this.destroy = ve.none, this.speed = 5;
	}
	load(e) {
		super.load(e), !O(e) && e.destroy !== void 0 && (this.destroy = e.destroy);
	}
	...
	constructor() {
		super(), this.animation = new xn(), this.value = 3;
	}
	load(e) {
		if (super.load(e), O(e)) return;
		let t = e.animation;
	...
	load(e) {
		O(e) || (e.color !== void 0 && (this.color = Gt.create(this.color, e.color)), e.width !== void 0 && (this.width = P(e.width)), e.opacity !== void 0 && (this.opacity = P(e.opacity)));
	}
}, wn = class extends Yt {
	constructor() {
	...
	load(e) {
		super.load(e), !O(e) && (e.opacityRate !== void 0 && (this.opacityRate = e.opacityRate), e.sizeRate !== void 0 && (this.sizeRate = e.sizeRate), e.velocityRate !== void 0 && (this.velocityRate = e.velocityRate));
	}
}, Tn = class {
	constructor(e, t) {
		this._engine = e, this._container = t, this.bounce = new $t(), this.collisions = new en(), this.color = new Gt(), this.color.value = "#fff", this.effect = new tn(), this.groups = {}, this.move = new fn(), this.number = new vn(), this.opacity = new mn(), this.reduceDuplicates = !1, this.shadow = new yn(), this.shape = new bn(), this.size = new Sn(), this.stroke = new Cn(), this.zIndex = new wn();
	}
	load(e) {
		if (O(e)) return;
...
var On = class {
	constructor(e, t) {
		this._findDefaultTheme = (e) => this.themes.find((t) => t.default.value && t.default.mode === e) ?? this.themes.find((e) => e.default.value && e.default.mode === q.any), this._importPreset = (e) => {
			this.load(this._engine.getPreset(e));
		}, this._engine = e, this._container = t, this.autoPlay = !0, this.background = new Ct(), this.backgroundMask = new Tt(), this.clear = !0, this.defaultThemes = {}, this.delay = 0, this.fullScreen = new Et(), this.detectRetina = !0, this.duration = 0, this.fpsLimit = 120, this.interactivity = new Ft(e, t), this.manualParticles = [], this.particles = Dn(this._engine, this._container), this.pauseOnBlur = !0, this.pauseOnOutsideViewport = !0, this.responsive = [], this.smooth = !1, this.style = {}, this.themes = [], this.zLayers = 100;
	}
	load(e) {
		if (O(e)) return;
		e.preset !== void 0 && Fe(e.preset, (e) => this._importPreset(e)), e.autoPlay !== void 0 && (this.autoPlay = e.autoPlay), e.clear !== void 0 && (this.clear = e.clear), e.key !== void 0 && (this.key = e.key), e.name !== void 0 && (this.name = e.name), e.delay !== void 0 && (this.delay = P(e.delay));
		...
		let r = e.fullScreen;
		re(r) ? this.fullScreen.enable = r : this.fullScreen.load(r), this.backgroundMask.load(e.backgroundMask), this.interactivity.load(e.interactivity), e.manualParticles && (this.manualParticles = e.manualParticles.map((e) => {
			let t = new It();
			return t.load(e), t;
		})), this.particles.load(e.particles), this.style = z(this.style, e.style), this._engine.loadOptions(this, e), e.smooth !== void 0 && (this.smooth = e.smooth);
		...
		this.defaultThemes.dark = this._findDefaultTheme(q.dark)?.name, this.defaultThemes.light = this._findDefaultTheme(q.light)?.name;
	}
	setResponsive(e, t, n) {
		this.load(n);
		let r = this.responsive.find((n) => n.mode === Lt.screen && screen ? n.maxWidth > screen.availWidth : n.maxWidth * t > e);
	...
	setTheme(e) {
		if (e) {
			...
			let e = Ee("(prefers-color-scheme: dark)")?.matches, t = this._findDefaultTheme(e ? q.dark : q.light);
			t && this.load(t.options);
		}
	}
}, kn;
...
function Nn(e) {
	if (!ke(e.outMode, e.checkModes)) return;
	let t = e.radius * 2;
...
var Pn = class {
	constructor(e, t) {
		this.container = t, this._calcPosition = (e, t, n, r = 0) => {
			for (let r of e.plugins.values()) {
				...
				if (e) return ie.create(e.x, e.y, n);
			}
			let i = e.canvas.size, a = he({
				size: i,
				position: t
			}), o = ie.create(a.x, a.y, n), s = this.getRadius(), c = this.options.move.outModes, l = (t) => {
				Nn({
					outMode: t,
					checkModes: [J.bounce],
					coord: o.x,
			...
			let e = me(this.direction).copy(), t = this.options.move;
			if (t.direction === C.inside || t.direction === C.outside) return e;
			let n = fe(N(t.angle.value)), r = fe(N(t.angle.offset)), i = {
				left: r - n * p,
				right: r + n * p
			};
			return t.straight || (e.angle += M(P(i.left, i.right))), t.random && typeof t.speed == "number" && (e.length *= A()), e;
			...
			let a = i.retries;
			if (a >= 0 && t > a) throw Error(`${f} particle is overlapping and can't be placed`);
			return !!this.container.particles.find((t) => I(e, t.position) < r + t.getRadius());
		}, this._getRollColor = (e) => {
			if (!e || !this.roll || !this.backColor && !this.roll.alter) return e;
			let t = this.roll.horizontal && this.roll.vertical ? 2 : 1, n = this.roll.horizontal ? Math.PI * p : 0;
			return Math.floor(((this.roll.angle ?? 0) + n) / (Math.PI / t)) % 2 ? this.backColor ? this.backColor : this.roll.alter ? _t(e, this.roll.alter.type, this.roll.alter.value) : e : e;
		}, this._initPosition = (e) => {
			let t = this.container, n = N(this.options.zIndex.value);
			this.position = this._calcPosition(t, e, j(n, 0, t.zLayers)), this.initialPosition = this.position.copy();
			let r = t.canvas.size;
			switch (this.moveCenter = {
				...Re(this.options.move.center, r),
				radius: this.options.move.center.radius ?? 0,
				mode: this.options.move.center.mode ?? ye.percent
			}, this.direction = pe(this.options.move.direction, this.position, this.moveCenter), this.options.move.direction) {
				case C.inside:
					this.outType = Y.inside;
					break;
				case C.outside:
					this.outType = Y.outside;
					break;
			}
			this.offset = k.origin;
		}, this._engine = e;
	}
	destroy(e) {
		...
		n?.reset(this), this._engine.dispatchEvent(G.particleDestroyed, {
	...
	getFillColor() {
		return this._getRollColor(this.bubble.color ?? nt(this.color));
	}
	getMass() {
		return this.getRadius() ** 2 * Math.PI * p;
	}
	getPosition() {
		return {
	...
	getStrokeColor() {
		return this._getRollColor(this.bubble.color ?? nt(this.strokeColor));
	}
	init(e, t, n, r) {
		let i = this.container, a = this._engine;
		this.id = e, this.group = r, this.effectClose = !0, this.effectFill = !0, this.shapeClose = !0, this.shapeFill = !0, this.pathRotation = !1, this.lastPathTime = 0, this.destroyed = !1, this.unbreakable = !1, this.isRotating = !1, this.rotation = 0, this.misplaced = !1, this.retina = { maxDistance: {} }, this.outType = Y.normal, this.ignoresResizeRatio = !0;
		let o = i.retina.pixelRatio, s = i.actualOptions, c = Dn(this._engine, i, s.particles), { reduceDuplicates: l } = c, u = c.effect.type, d = c.shape.type;
		this.effect = B(u, this.id, l), this.shape = B(d, this.id, l);
		let f = c.effect, p = c.shape;
		if (n) {
			if (n.effect?.type) {
				let e = n.effect.type, t = B(e, this.id, l);
				t && (this.effect = t, f.load(n.effect));
			}
			if (n.shape?.type) {
				let e = n.shape.type, t = B(e, this.id, l);
				t && (this.shape = t, p.load(n.shape));
			}
		}
		if (this.effect === "random") {
			let e = [...this.container.effectDrawers.keys()];
			this.effect = e[Math.floor(A() * e.length)];
		}
		if (this.shape === "random") {
			let e = [...this.container.shapeDrawers.keys()];
			this.shape = e[Math.floor(A() * e.length)];
		}
		this.effectData = jn(this.effect, f, this.id, l), this.shapeData = Mn(this.shape, p, this.id, l), c.load(n);
		let h = this.effectData;
		h && c.load(h.particles);
		let g = this.shapeData;
		g && c.load(g.particles);
		let _ = new Ft(a, i);
		_.load(i.actualOptions.interactivity), _.load(c.interactivity), this.interactivity = _, this.effectFill = h?.fill ?? c.effect.fill, this.effectClose = h?.close ?? c.effect.close, this.shapeFill = g?.fill ?? c.shape.fill, this.shapeClose = g?.close ?? c.shape.close, this.options = c;
		let v = this.options.move.path;
		this.pathDelay = N(v.delay.value) * m, v.generator && (this.pathGenerator = this._engine.getPathGenerator(v.generator), this.pathGenerator && i.addPath(v.generator, this.pathGenerator) && this.pathGenerator.init(i)), i.retina.initParticle(this), this.size = Ie(this.options.size, o), this.bubble = { inRange: !1 }, this.slow = {
			inRange: !1,
			factor: 1
		}, this._initPosition(t), this.initialVelocity = this._calculateVelocity(), this.velocity = this.initialVelocity.copy(), this.moveDecay = 1 - N(this.options.move.decay);
		let y = i.particles;
		y.setLastZIndex(this.position.z), this.zIndexFactor = this.position.z / i.zLayers, this.sides = 24;
		let b = i.effectDrawers.get(this.effect);
		b || (b = this._engine.getEffectDrawer(this.effect), b && i.effectDrawers.set(this.effect, b)), b?.loadEffect && b.loadEffect(this);
		let x = i.shapeDrawers.get(this.shape);
		x || (x = this._engine.getShapeDrawer(this.shape), x && i.shapeDrawers.set(this.shape, x)), x?.loadShape && x.loadShape(this);
		let S = x?.getSidesCount;
		S && (this.sides = S(this)), this.spawning = !1, this.shadowColor = V(this._engine, this.options.shadow.color);
		for (let e of y.updaters) e.init(this);
		for (let e of y.movers) e.init?.(this);
		b?.particleInit?.(i, this), x?.particleInit?.(i, this);
...
var Ln = class {
...
}, Rn = class e extends Ln {
	constructor(e, t, n) {
		super(e, t, In.circle), this.radius = n;
	}
	contains(e) {
		return I(e, this.position) <= this.radius;
	}
	intersects(t) {
		...
		if (t instanceof e || t.type === In.circle) return a + t.radius > Math.sqrt(i.x ** 2 + i.y ** 2);
		if (t instanceof zn || t.type === In.rectangle) {
	...
	constructor(e, t, n, r) {
		super(e, t, In.rectangle), this.size = {
			height: r,
			width: n
		};
	}
	...
	intersects(t) {
		if (t instanceof Rn) return t.intersects(this);
	...
	constructor(t, n) {
		this.rectangle = t, this.capacity = n, this._subdivide = () => {
			...
			for (let o = 0; o < 4; o++) {
				let s = o % 2;
				this._subs.push(new e(new zn(t + r * p * s, n + i * p * (Math.round(o * p) - s), r * p, i * p), a));
			}
			this._divided = !0;
	...
	query(e, t) {
		...
		for (let r of this._points) !e.contains(r.position) && I(e.position, r.position) > r.particle.getRadius() && (!t || t(r.particle)) || n.push(r.particle);
		if (this._divided) for (let r of this._subs) n.push(...r.query(e, t));
		return n;
	}
	queryCircle(e, t, n) {
		return this.query(new Rn(e.x, e.y, t), n);
	}
	queryRectangle(e, t, n) {
		return this.query(new zn(e.x, e.y, t.width, t.height), n);
	}
}, Vn = (e) => {
	let { height: t, width: n } = e;
	return new zn(te * n, te * t, ne * n, ne * t);
}, Hn = class {
	constructor(e, t) {
		this._addToPool = (...e) => {
			this._pool.push(...e);
			...
			try {
				...
				i ||= new Pn(this._engine, this._container), i.init(this._nextId, e, t, n);
				let a = !0;
				return r && (a = r(i)), a ? (this._array.push(i), this._zArray.push(i), this._nextId++, this._engine.dispatchEvent(G.particleAdded, {
					container: this._container,
					data: { particle: i }
				}), i) : void 0;
			} catch (e) {
				Se().warning(`${f} adding particle: ${e}`);
			}
		}, this._removeParticle = (e, t, n) => {
			let r = this._array[e];
			if (!r || r.group !== t) return !1;
			let i = this._zArray.indexOf(r);
			return this._array.splice(e, 1), this._zArray.splice(i, 1), r.destroy(n), this._engine.dispatchEvent(G.particleRemoved, {
				container: this._container,
				data: { particle: r }
			}), this._addToPool(r), !0;
		}, this._engine = e, this._container = t, this._nextId = 0, this._array = [], this._zArray = [], this._pool = [], this._limit = 0, this._groupLimits = /* @__PURE__ */ new Map(), this._needsSort = !1, this._lastZIndex = 0, this._interactionManager = new An(e, t), this._pluginsInitialized = !1;
		let n = t.canvas.size;
		this.quadTree = new Bn(Vn(n), 4), this.movers = [], this.updaters = [];
	}
	get count() {
	...
	addManualParticles() {
		...
		e.actualOptions.manualParticles.forEach((t) => this.addParticle(t.position ? Re(t.position, e.canvas.size) : void 0, t.options));
	}
	addParticle(e, t, n, r) {
	...
	update(e) {
		...
		this.quadTree = new Bn(Vn(t.canvas.size), 4);
		...
		for (let t of this._array) {
			...
			this.quadTree.insert(new Fn(t.getPosition(), t));
		}
		if (n.size) {
			let e = (e) => !n.has(e);
			this._array = this.filter(e), this._zArray = this._zArray.filter(e);
			for (let e of n) this._engine.dispatchEvent(G.particleRemoved, {
				container: this._container,
				data: { particle: e }
			});
			this._addToPool(...n);
	...
	init() {
		let e = this.container, t = e.actualOptions;
		this.pixelRatio = !t.detectRetina || we() ? 1 : devicePixelRatio, this.reduceFactor = 1;
		...
		this.maxSpeed = N(a.gravity.maxSpeed) * n, this.sizeAnimationSpeed = N(i.size.animation.speed) * n;
	}
	initParticle(e) {
		let t = e.options, n = this.pixelRatio, r = t.move, i = r.distance, a = e.retina;
		a.moveDrift = N(r.drift) * n, a.moveSpeed = N(r.speed) * n, a.sizeAnimationSpeed = N(t.size.animation.speed) * n;
		let o = a.maxDistance;
		o.horizontal = i.horizontal === void 0 ? void 0 : i.horizontal * n, o.vertical = i.vertical === void 0 ? void 0 : i.vertical * n, a.maxSpeed = N(r.gravity.maxSpeed) * n;
	}
};
//#endregion
...
function Wn(e, t = 60, n = !1) {
	return {
		value: e,
		factor: n ? 60 / t : 60 * e / m
	};
}
function Gn(e, t, ...n) {
	let r = new On(e, t);
...
var Kn = class {
	constructor(e, t, n) {
		this._intersectionManager = (e) => {
			if (!(!X(this) || !this.actualOptions.pauseOnOutsideViewport)) for (let t of e) t.target === this.interactivity.element && (t.isIntersecting ? this.play() : this.pause());
		}, this._nextFrame = (e) => {
			try {
				...
				let t = Wn(e - this._lastFrameTime, this.fpsLimit, this._smooth);
				...
				Se().error(`${f} in animation loop`, e);
			}
		}, this._engine = e, this.id = Symbol(t), this.fpsLimit = 120, this._smooth = !1, this._delay = 0, this._duration = 0, this._lifeTime = 0, this._firstStart = !0, this.started = !1, this.destroyed = !1, this._paused = !0, this._lastFrameTime = 0, this.zLayers = 100, this.pageHidden = !1, this._clickHandlers = /* @__PURE__ */ new Map(), this._sourceOptions = n, this._initialSourceOptions = n, this.retina = new Un(this), this.canvas = new bt(this, this._engine), this.particles = new Hn(this._engine, this), this.pathGenerators = /* @__PURE__ */ new Map(), this.interactivity = { mouse: {
			clicking: !1,
			inside: !1
		} }, this.plugins = /* @__PURE__ */ new Map(), this.effectDrawers = /* @__PURE__ */ new Map(), this.shapeDrawers = /* @__PURE__ */ new Map(), this._options = Gn(this._engine, this), this.actualOptions = Gn(this._engine, this), this._eventListeners = new St(this), this._intersectionObserver = De((e) => this._intersectionManager(e)), this._engine.dispatchEvent(G.containerBuilt, { container: this });
	}
	get animationStatus() {
		...
		let n = (t, n, r) => {
			...
			if (X(this)) {
				if (c && !l) {
					...
					let i = this.canvas.element, a = i ? i.getBoundingClientRect() : void 0;
					n(e, {
						x: r.clientX - (a ? a.left : 0),
						y: r.clientY - (a ? a.top : 0)
					}, Math.max(r.radiusX, r.radiusY));
				}
				c = !1, l = !1;
			}
		}, s = () => {
	...
	addPath(e, t, n = !1) {
		return !X(this) || !n && this.pathGenerators.has(e) ? !1 : (this.pathGenerators.set(e, t), !0);
	}
	alive() {
	...
	destroy(e = !0) {
		if (X(this)) {
			...
			this._engine.dispatchEvent(G.containerDestroyed, { container: this });
		}
	}
	draw(e) {
		if (!X(this)) return;
		...
		this._drawAnimationFrame = se((e) => n(e));
	}
	async export(e, t = {}) {
		...
		Se().error(`${f} - Export plugin with type ${e} not found`);
	}
	handleClickMode(e) {
		if (X(this)) {
	...
	async init() {
		...
		await this.particles.initPlugins(), this._options = Gn(this._engine, this, this._initialSourceOptions, this.sourceOptions), this.actualOptions = Gn(this._engine, this, this._options);
		...
		this.zLayers = r, this._duration = N(i) * m, this._delay = N(a) * m, this._lifeTime = 0, this.fpsLimit = o > 0 ? o : 120, this._smooth = s;
		...
		this._engine.dispatchEvent(G.containerInit, { container: this }), await this.particles.init(), this.particles.setDensity();
		for (let e of this.plugins.values()) e.particlesSetup?.();
		this._engine.dispatchEvent(G.particlesSetup, { container: this });
	}
	async loadTheme(e) {
		X(this) && (this._currentTheme = e, await this.refresh());
	}
	pause() {
		if (X(this) && (this._drawAnimationFrame !== void 0 && (ce(this._drawAnimationFrame), delete this._drawAnimationFrame), !this._paused)) {
			for (let e of this.plugins.values()) e.pause?.();
			this.pageHidden || (this._paused = !0), this._engine.dispatchEvent(G.containerPaused, { container: this });
		}
	}
	play(e) {
		if (!X(this)) return;
		...
		this._engine.dispatchEvent(G.containerPlay, { container: this }), this.draw(t ?? !1);
	}
	async refresh() {
		if (X(this)) return this.stop(), this.start();
	}
	async reset(e) {
		if (X(this)) return this._initialSourceOptions = e, this._sourceOptions = e, this._options = Gn(this._engine, this, this._initialSourceOptions, this.sourceOptions), this.actualOptions = Gn(this._engine, this, this._options), this.refresh();
	}
	async start() {
		!X(this) || this.started || (await this.init(), this.started = !0, await new Promise((e) => {
			let t = async () => {
				...
				this._engine.dispatchEvent(G.containerStarted, { container: this }), this.play(), e();
			};
	...
	stop() {
		if (!(!X(this) || !this.started)) {
			...
			this._sourceOptions = this._options, this._engine.dispatchEvent(G.containerStopped, { container: this });
		}
	}
	updateActualOptions() {
...
async function Jn(e, t, n, r = !1) {
	let i = t.get(e);
	return (!i || r) && (i = await Promise.all([...n.values()].map((t) => t(e))), t.set(e, i)), i;
}
async function Yn(e) {
	let t = B(e.url, e.index);
	if (!t) return e.fallback;
	let n = await fetch(t);
	return n.ok ? await n.json() : (Se().error(`${f} ${n.status} while retrieving config file`), e.fallback);
}
var Xn = (t) => {
	let n;
	if (t instanceof HTMLCanvasElement || t.tagName.toLowerCase() === "canvas") n = t, n.dataset.generated || (n.dataset[e] = b);
	else {
		let r = t.getElementsByTagName(x);
		r.length ? (n = r[0], n.dataset[e] = b) : (n = document.createElement(x), n.dataset[e] = y, t.appendChild(n));
	}
	let r = "100%";
	return n.style.width || (n.style.width = r), n.style.height || (n.style.height = r), n;
}, Zn = (t, n) => {
	let r = n ?? document.getElementById(t);
	return r || (r = document.createElement("div"), r.id = t, r.dataset[e] = y, document.body.append(r), r);
}, Qn = class {
	constructor() {
		this._configs = /* @__PURE__ */ new Map(), this._domArray = [], this._eventDispatcher = new qn(), this._initialized = !1, this.plugins = [], this.colorManagers = /* @__PURE__ */ new Map(), this.easingFunctions = /* @__PURE__ */ new Map(), this._initializers = {
	...
	addConfig(e) {
		...
		this._configs.set(t, e), this._eventDispatcher.dispatchEvent(G.configAdded, { data: {
			name: t,
			config: e
		} });
	}
	...
	async getInteractors(e, t = !1) {
		return Jn(e, this.interactors, this._initializers.interactors, t);
	}
	async getMovers(e, t = !1) {
		return Jn(e, this.movers, this._initializers.movers, t);
	}
	getPathGenerator(e) {
	...
	async getUpdaters(e, t = !1) {
		return Jn(e, this.updaters, this._initializers.updaters, t);
	}
	init() {
		this._initialized ||= !0;
	}
	...
	async load(e) {
		let t = e.id ?? e.element?.id ?? `tsparticles${Math.floor(A() * 1e4)}`, { index: n, url: r } = e, i = B(r ? await Yn({
			fallback: e.options,
			url: r,
			index: n
		}) : e.options, n), { items: a } = this, o = a.findIndex((e) => e.id.description === t), s = new Kn(this, t, i);
		if (o >= 0) {
			let e = this.item(o), t = +!!e;
			e && !e.destroyed && e.destroy(!1), a.splice(o, t, s);
		} else a.push(s);
		let c = Xn(Zn(t, e.element));
	...
	setOnClickHandler(e) {
		let { items: t } = this;
		if (!t.length) throw Error(`${f} can only set click handlers after calling tsParticles.load()`);
...
//#endregion
//#region node_modules/@tsparticles/engine/browser/Core/Utils/ParticlesInteractorBase.js
var er = class {
	constructor(e) {
		this.type = kn.particles, this.container = e;
	}
}, tr;
(function(e) {
...
var nr = $n();
we() || (window.tsParticles = nr);
//#endregion
//#region node_modules/@tsparticles/move-base/browser/Utils.js
var rr = .5, ir = 2, Z = 0, Q = 1, ar = 60, or = 0, sr = .01, cr = Math.PI * ir;
function lr(e) {
	let t = e.initialPosition, { dx: n, dy: r } = F(t, e.position), i = Math.abs(n), a = Math.abs(r), { maxDistance: o } = e.retina, s = o.horizontal, c = o.vertical;
	if (!(!s && !c)) {
		if ((((s && i >= s) ?? !1) || ((c && a >= c) ?? !1)) && !e.misplaced) e.misplaced = !!s && i > s || !!c && a > c, s && (e.velocity.x = e.velocity.y * rr - e.velocity.x), c && (e.velocity.y = e.velocity.x * rr - e.velocity.y);
		else if ((!s || i < s) && (!c || a < c) && e.misplaced) e.misplaced = !1;
		else if (e.misplaced) {
			let n = e.position, r = e.velocity;
			s && (n.x < t.x && r.x < Z || n.x > t.x && r.x > Z) && (r.x *= -A()), c && (n.y < t.y && r.y < Z || n.y > t.y && r.y > Z) && (r.y *= -A());
		}
	}
}
function ur(e, t, n, r, i, a, o) {
	fr(e, o);
	let s = e.gravity, c = s?.enable && s.inverse ? -Q : Q;
	i && n && (e.velocity.x += i * o.factor / (ar * n)), s?.enable && n && (e.velocity.y += c * (s.acceleration * o.factor) / (ar * n));
	let l = e.moveDecay;
	e.velocity.multTo(l);
	let u = e.velocity.mult(n);
	s?.enable && r > Z && (!s.inverse && u.y >= Z && u.y >= r || s.inverse && u.y <= Z && u.y <= -r) && (u.y = c * r, n && (e.velocity.y = u.y / n));
	let d = e.options.zIndex, f = (Q - e.zIndexFactor) ** d.velocityRate;
...
function dr(e, t, n) {
	let r = e.container;
	if (!e.spin) return;
	let i = e.spin.direction === tr.clockwise, a = {
	...
	let o = Math.max(r.canvas.size.width, r.canvas.size.height), s = o * rr;
	e.spin.radius > s ? (e.spin.radius = s, e.spin.acceleration *= -Q) : e.spin.radius < or && (e.spin.radius = or, e.spin.acceleration *= -Q), e.spin.angle += t * sr * (Q - e.spin.radius / o);
}
function fr(e, t) {
	...
	r && e.velocity.addTo(r), n.clamp && (e.velocity.x = j(e.velocity.x, -Q, Q), e.velocity.y = j(e.velocity.y, -Q, Q)), e.lastPathTime -= e.pathDelay;
}
function pr(e) {
	return e.slow.inRange ? e.slow.factor : Q;
}
function mr(e) {
	...
	}, o = I(e.getPosition(), a), s = N(n.acceleration);
	e.retina.spinAcceleration = s * t.retina.pixelRatio, e.spin = {
		center: a,
		direction: e.velocity.x >= Z ? tr.clockwise : tr.counterClockwise,
		angle: A() * cr,
		radius: o,
...
var hr = 2, gr = 1, _r = 1, vr = class {
	init(e) {
		let t = e.options.move.gravity;
		e.gravity = {
			enable: t.enable,
			acceleration: N(t.acceleration),
			inverse: t.inverse
		}, mr(e);
	}
	isEnabled(e) {
	...
	move(e, t) {
		...
		e.retina.moveSpeed ??= N(r.speed) * a, e.retina.moveDrift ??= N(e.options.move.drift) * a;
		let o = pr(e), s = i.retina.reduceFactor, c = e.retina.moveSpeed, l = e.retina.moveDrift, u = de(n.size.value) * a, d = r.size ? e.getRadius() / u : gr, f = t.factor || _r, p = c * d * o * f / hr, m = e.retina.maxSpeed ?? i.retina.maxSpeed;
		r.spin.enable ? dr(e, p, s) : ur(e, r, p, m, l, s, t), lr(e);
	}
};
//#endregion
//#region node_modules/@tsparticles/move-base/browser/index.js
async function yr(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addMover("base", () => Promise.resolve(new vr()), t);
}
//#endregion
//#region node_modules/@tsparticles/shape-circle/browser/Utils.js
var br = Math.PI * 2, xr = 0, Sr = {
	x: 0,
	y: 0
};
function Cr(e) {
...
async function Or(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addShape(new Dr(), t);
}
//#endregion
...
var kr = class {
	...
	init(e) {
		let t = qe(this._engine, e.options.color, e.id, e.options.reduceDuplicates);
		t && (e.color = rt(t, e.options.color.animation, this._container.retina.reduceFactor));
	}
	isEnabled(e) {
...
async function Ar(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addParticleUpdater("color", (t) => Promise.resolve(new kr(t, e)), t);
}
//#endregion
...
var Mr = /^#?([a-f\d])([a-f\d])([a-f\d])([a-f\d])?$/i, Nr = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})?$/i, Pr = 16, Fr = 1, Ir = 255, Lr = class {
	constructor() {
	...
	_parseString(e) {
		...
		let t = e.replace(Mr, (e, t, n, r, i) => t + t + n + n + r + r + (i === void 0 ? "" : i + i)), n = Nr.exec(t);
		return n ? {
			a: n[jr.a] === void 0 ? Fr : parseInt(n[jr.a], Pr) / Ir,
			b: parseInt(n[jr.b], Pr),
			g: parseInt(n[jr.g], Pr),
			r: parseInt(n[jr.r], Pr)
		} : void 0;
	}
};
//#endregion
...
async function Rr(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addColorManager(new Lr(), t);
}
//#endregion
...
var Br = class {
	...
	handleColor(e) {
		...
		if (t.h !== void 0 && t.s !== void 0 && t.l !== void 0) return H(t);
	}
	handleRangeColor(e) {
		let t = e.value.hsl ?? e.value;
		if (t.h !== void 0 && t.l !== void 0) return H({
			h: N(t.h),
			l: N(t.l),
			s: N(t.s)
		});
	...
	parseString(e) {
		...
		return t ? Xe({
			a: t.length > 4 ? ge(t[zr.a]) : 1,
			h: parseInt(t[zr.h], 10),
			l: parseInt(t[zr.l], 10),
			s: parseInt(t[zr.s], 10)
		}) : void 0;
	}
};
//#endregion
//#region node_modules/@tsparticles/plugin-hsl-color/browser/index.js
async function Vr(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addColorManager(new Br(), t);
}
//#endregion
...
var Hr = class {
	...
	init(e) {
		...
		e.opacity = Ie(t, 1);
		let n = t.animation;
		n.enable && (e.opacity.velocity = N(n.speed) / 100 * this.container.retina.reduceFactor, n.sync || (e.opacity.velocity *= A()));
	}
	isEnabled(e) {
	...
	update(e, t) {
		!this.isEnabled(e) || !e.opacity || Be(e, e.opacity, !0, e.options.opacity.animation.destroy, t);
	}
};
//#endregion
//#region node_modules/@tsparticles/updater-opacity/browser/index.js
async function Ur(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addParticleUpdater("opacity", (e) => Promise.resolve(new Hr(e)), t);
}
//#endregion
//#region node_modules/@tsparticles/updater-out-modes/browser/Utils.js
var Wr = 0, Gr = 0;
function Kr(e) {
	if (e.outMode !== J.bounce && e.outMode !== J.split || e.direction !== R.left && e.direction !== R.right) return;
	e.bounds.right < Gr && e.direction === R.left ? e.particle.position.x = e.size + e.offset.x : e.bounds.left > e.canvasSize.width && e.direction === R.right && (e.particle.position.x = e.canvasSize.width - e.size - e.offset.x);
	let t = e.particle.velocity.x, n = !1;
	if (e.direction === R.right && e.bounds.right >= e.canvasSize.width && t > Wr || e.direction === R.left && e.bounds.left <= Gr && t < Wr) {
		let t = N(e.particle.options.bounce.horizontal.value);
		e.particle.velocity.x *= -t, n = !0;
	}
	if (!n) return;
	let r = e.offset.x + e.size;
	e.bounds.right >= e.canvasSize.width && e.direction === R.right ? e.particle.position.x = e.canvasSize.width - r : e.bounds.left <= Gr && e.direction === R.left && (e.particle.position.x = r), e.outMode === J.split && e.particle.destroy();
}
function qr(e) {
	if (e.outMode !== J.bounce && e.outMode !== J.split || e.direction !== R.bottom && e.direction !== R.top) return;
	e.bounds.bottom < Gr && e.direction === R.top ? e.particle.position.y = e.size + e.offset.y : e.bounds.top > e.canvasSize.height && e.direction === R.bottom && (e.particle.position.y = e.canvasSize.height - e.size - e.offset.y);
	let t = e.particle.velocity.y, n = !1;
	if (e.direction === R.bottom && e.bounds.bottom >= e.canvasSize.height && t > Wr || e.direction === R.top && e.bounds.top <= Gr && t < Wr) {
		let t = N(e.particle.options.bounce.vertical.value);
		e.particle.velocity.y *= -t, n = !0;
	}
	if (!n) return;
	let r = e.offset.y + e.size;
	e.bounds.bottom >= e.canvasSize.height && e.direction === R.bottom ? e.particle.position.y = e.canvasSize.height - r : e.bounds.top <= Gr && e.direction === R.top && (e.particle.position.y = r), e.outMode === J.split && e.particle.destroy();
}
//#endregion
//#region node_modules/@tsparticles/updater-out-modes/browser/BounceOutMode.js
var Jr = class {
	constructor(e) {
		this.container = e, this.modes = [J.bounce, J.split];
	}
	update(e, t, n, r) {
		...
		if (a) return;
		let o = e.getPosition(), s = e.offset, c = e.getRadius(), l = Pe(o, c), u = i.canvas.size;
		Kr({
			particle: e,
			outMode: r,
			direction: t,
	...
	constructor(e) {
		this.container = e, this.modes = [J.destroy];
	}
	update(e, t, n, r) {
		if (!this.modes.includes(r)) return;
		...
		switch (e.outType) {
			case Y.normal:
			case Y.outside:
				if (Me(e.position, i.canvas.size, k.origin, e.getRadius(), t)) return;
				break;
			case Y.inside: {
				let { dx: t, dy: n } = F(e.position, e.moveCenter), { x: r, y: i } = e.velocity;
				if (r < Yr && t > e.moveCenter.radius || i < Yr && n > e.moveCenter.radius || r >= Yr && t < -e.moveCenter.radius || i >= Yr && n < -e.moveCenter.radius) return;
				break;
			}
		}
	...
	constructor(e) {
		this.container = e, this.modes = [J.none];
	}
	update(e, t, n, r) {
		if (!this.modes.includes(r) || ((e.options.move.distance.horizontal && (t === R.left || t === R.right)) ?? (e.options.move.distance.vertical && (t === R.top || t === R.bottom)))) return;
		let i = e.options.move.gravity, a = this.container, o = a.canvas.size, s = e.getRadius();
		if (i.enable) {
			let n = e.position;
			(!i.inverse && n.y > o.height + s && t === R.bottom || i.inverse && n.y < -s && t === R.top) && a.particles.remove(e);
		} else {
			if (e.velocity.y > Zr && e.position.y <= o.height + s || e.velocity.y < Zr && e.position.y >= -s || e.velocity.x > Zr && e.position.x <= o.width + s || e.velocity.x < Zr && e.position.x >= -s) return;
			Me(e.position, a.canvas.size, k.origin, s, t) || a.particles.remove(e);
		}
	}
}, $r = 0, ei = 0, ti = class {
	constructor(e) {
		this.container = e, this.modes = [J.out];
	}
	update(e, t, n, r) {
		if (!this.modes.includes(r)) return;
		let i = this.container;
		switch (e.outType) {
			case Y.inside: {
				let { x: t, y: n } = e.velocity, r = k.origin;
				r.length = e.moveCenter.radius, r.angle = e.velocity.angle + Math.PI, r.addTo(k.create(e.moveCenter));
				let { dx: a, dy: o } = F(e.position, r);
				if (t <= $r && a >= ei || n <= $r && o >= ei || t >= $r && a <= ei || n >= $r && o <= ei) return;
				e.position.x = Math.floor(M({
					min: 0,
					max: i.canvas.size.width
				})), e.position.y = Math.floor(M({
					min: 0,
					max: i.canvas.size.height
				}));
				let { dx: s, dy: c } = F(e.position, e.moveCenter);
			...
			default:
				...
				switch (e.outType) {
					case Y.outside: {
						...
						})) + e.moveCenter.x, e.position.y = Math.floor(M({
							min: -e.moveCenter.radius,
							max: e.moveCenter.radius
						})) + e.moveCenter.y;
						let { dx: t, dy: n } = F(e.position, e.moveCenter);
					...
					case Y.normal: {
						...
						}, o = e.getRadius(), s = Pe(e.position, o);
						t === R.right && s.left > r.width + e.offset.x ? (e.position.x = a.left, e.initialPosition.x = e.position.x, n || (e.position.y = A() * r.height, e.initialPosition.y = e.position.y)) : t === R.left && s.right < -e.offset.x && (e.position.x = a.right, e.initialPosition.x = e.position.x, n || (e.position.y = A() * r.height, e.initialPosition.y = e.position.y)), t === R.bottom && s.top > r.height + e.offset.y ? (n || (e.position.x = A() * r.width, e.initialPosition.x = e.position.x), e.position.y = a.top, e.initialPosition.y = e.position.y) : t === R.top && s.bottom < -e.offset.y && (n || (e.position.x = A() * r.width, e.initialPosition.x = e.position.x), e.position.y = a.bottom, e.initialPosition.y = e.position.y);
						break;
					}
				}
				break;
...
}, ni = (e, t) => e.default === t || e.bottom === t || e.left === t || e.right === t || e.top === t, ri = class {
	constructor(e) {
		this._addUpdaterIfMissing = (e, t, n) => {
			let r = e.options.move.outModes;
			!this.updaters.has(t) && ni(r, t) && this.updaters.set(t, n(this.container));
	...
	init(e) {
		this._addUpdaterIfMissing(e, J.bounce, (e) => new Jr(e)), this._addUpdaterIfMissing(e, J.out, (e) => new ti(e)), this._addUpdaterIfMissing(e, J.destroy, (e) => new Xr(e)), this._addUpdaterIfMissing(e, J.none, (e) => new Qr(e));
	}
	isEnabled(e) {
	...
	update(e, t) {
		...
		this._updateOutMode(e, t, n.bottom ?? n.default, R.bottom), this._updateOutMode(e, t, n.left ?? n.default, R.left), this._updateOutMode(e, t, n.right ?? n.default, R.right), this._updateOutMode(e, t, n.top ?? n.default, R.top);
	}
};
//#endregion
//#region node_modules/@tsparticles/updater-out-modes/browser/index.js
async function ii(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addParticleUpdater("outModes", (e) => Promise.resolve(new ri(e)), t);
}
//#endregion
...
var oi = class {
	...
	parseString(e) {
		...
		return t ? {
			a: t.length > 4 ? ge(t[ai.a]) : 1,
			b: parseInt(t[ai.b], 10),
			g: parseInt(t[ai.g], 10),
			r: parseInt(t[ai.r], 10)
		} : void 0;
	}
};
//#endregion
...
async function si(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addColorManager(new oi(), t);
}
//#endregion
...
var $ = 0, ci = class {
	init(e) {
		...
		n.enable && (e.size.velocity = (e.retina.sizeAnimationSpeed ?? t.retina.sizeAnimationSpeed) / 100 * t.retina.reduceFactor, n.sync || (e.size.velocity *= A()));
	}
	isEnabled(e) {
		return !e.destroyed && !e.spawning && e.size.enable && ((e.size.maxLoops ?? $) <= $ || (e.size.maxLoops ?? $) > $ && (e.size.loops ?? $) < (e.size.maxLoops ?? $));
	}
	reset(e) {
		e.size.loops = $;
	}
	update(e, t) {
		this.isEnabled(e) && Be(e, e.size, !0, e.options.size.animation.destroy, t);
	}
};
//#endregion
//#region node_modules/@tsparticles/updater-size/browser/index.js
async function li(e, t = !0) {
	e.checkVersion("3.9.1"), await e.addParticleUpdater("size", () => Promise.resolve(new ci()), t);
}
//#endregion
//#region node_modules/@tsparticles/basic/browser/index.js
async function ui(e, t = !0) {
	e.checkVersion("3.9.1"), await Rr(e, !1), await Vr(e, !1), await si(e, !1), await yr(e, !1), await Or(e, !1), await Ar(e, !1), await Ur(e, !1), await ii(e, !1), await li(e, !1), await e.refresh(t);
}
//#endregion
//#region node_modules/@tsparticles/interaction-particles-links/browser/CircleWarp.js
var di = 2, fi = class extends Rn {
	constructor(e, t, n, r) {
	...
	intersects(e) {
		...
		if (n.radius !== void 0) {
			let e = new Rn(r.x, r.y, n.radius * di);
			return super.intersects(e);
		} else if (t.size !== void 0) {
			let e = new zn(r.x, r.y, t.size.width * di, t.size.height * di);
			return super.intersects(e);
		}
		return !1;
	}
	...
	constructor() {
		this.blur = 5, this.color = new K(), this.color.value = "#000", this.enable = !1;
	}
	load(e) {
		O(e) || (e.blur !== void 0 && (this.blur = e.blur), this.color = K.create(this.color, e.color), e.enable !== void 0 && (this.enable = e.enable));
	}
}, mi = class {
	constructor() {
	...
	load(e) {
		O(e) || (e.color !== void 0 && (this.color = K.create(this.color, e.color)), e.enable !== void 0 && (this.enable = e.enable), e.frequency !== void 0 && (this.frequency = e.frequency), e.opacity !== void 0 && (this.opacity = e.opacity));
	}
}, hi = class {
	constructor() {
		this.blink = !1, this.color = new K(), this.color.value = "#fff", this.consent = !1, this.distance = 100, this.enable = !1, this.frequency = 1, this.opacity = 1, this.shadow = new pi(), this.triangles = new mi(), this.width = 1, this.warp = !1;
	}
	load(e) {
		O(e) || (e.id !== void 0 && (this.id = e.id), e.blink !== void 0 && (this.blink = e.blink), this.color = K.create(this.color, e.color), e.consent !== void 0 && (this.consent = e.consent), e.distance !== void 0 && (this.distance = e.distance), e.enable !== void 0 && (this.enable = e.enable), e.frequency !== void 0 && (this.frequency = e.frequency), e.opacity !== void 0 && (this.opacity = e.opacity), this.shadow.load(e.shadow), this.triangles.load(e.triangles), e.width !== void 0 && (this.width = e.width), e.warp !== void 0 && (this.warp = e.warp));
	}
}, gi = 2, _i = 1, vi = {
	x: 0,
	y: 0
...
function bi(e, t, n, r, i) {
	let { dx: a, dy: o, distance: s } = F(e, t);
	if (!i || s <= n) return s;
	let c = {
		x: Math.abs(a),
		y: Math.abs(o)
	...
	return Math.sqrt(l.x ** gi + l.y ** gi);
}
var xi = class extends er {
	constructor(e, t) {
		super(e), this._setColor = (e) => {
			...
			if (r) return;
			let i = n.color;
			r = tt(this._engine, i, n.blink, n.consent), n.id === void 0 ? t.particles.linksColor = r : t.particles.linksColors.set(n.id, r);
	...
	interact(e) {
		...
		if (t.x < vi.x || t.y < vi.y || t.x > r.width || t.y > r.height) return;
		let i = e.options.links, a = i.opacity, o = e.retina.linksDistance ?? yi, s = i.warp, c;
		c = s ? new fi(t.x, t.y, o, r) : new Rn(t.x, t.y, o);
		...
		for (let n of l) {
			...
			let l = n.getPosition();
			if (l.x < vi.x || l.y < vi.y || l.x > r.width || l.y > r.height) continue;
			let u = bi(t, l, o, r, s && c.warp);
			if (u > o) continue;
			let d = (_i - u / o) * a;
...
async function Si(e, t = !0) {
	await e.addInteractor("particlesLinks", async (t) => Promise.resolve(new xi(t, e)), t);
}
//#endregion
...
function wi(e) {
	...
	if (I(n, r) <= a) st(o, n, r), t = !0;
		...
		e && i && (st(o, n, e), st(o, r, i), t = !0);
	}
	if (!t) return;
	o.lineWidth = c, l.enable && (o.globalCompositeOperation = l.composite), o.strokeStyle = U(u, d);
	let { shadow: p } = f;
	if (p.enable) {
		let e = V(i, p.color);
		e && (o.shadowBlur = p.blur, o.shadowColor = U(e));
	}
	o.stroke();
}
function Ti(e) {
	let { context: t, pos1: n, pos2: r, pos3: i, backgroundMask: a, colorTriangle: o, opacityTriangle: s } = e;
	Ci(t, n, r, i), a.enable && (t.globalCompositeOperation = a.composite), t.fillStyle = U(o, s), t.fill();
}
function Ei(e) {
	return e.sort((e, t) => e - t), e.join("_");
}
function Di(e, t) {
	let n = Ei(e.map((e) => e.id)), r = t.get(n);
	return r === void 0 && (r = A(), t.set(n, r)), r;
}
//#endregion
//#region node_modules/@tsparticles/interaction-particles-links/browser/LinkInstance.js
var Oi = 0, ki = 0, Ai = 0, ji = .5, Mi = 1, Ni = class {
	constructor(e, t) {
		this._drawLinkLine = (e, t) => {
			...
			r.canvas.draw((t) => {
				...
				if (u?.enable) {
					let e = u.frequency, t = V(this._engine, u.color);
					A() < e && t && (l = t, c = N(u.opacity));
				}
				if (l ||= et(e, a, n.id === void 0 ? r.particles.linksColor : r.particles.linksColors.get(n.id)), !l) return;
				let d = e.retina.linksWidth ?? ki, f = e.retina.linksDistance ?? Ai, { backgroundMask: p } = i;
				wi({
					context: t,
					width: d,
					begin: o,
			...
			let i = r.triangles;
			if (!i.enable) return;
			let a = this._container, o = a.actualOptions, s = t.destination, c = n.destination, l = i.opacity ?? (t.opacity + n.opacity) * ji;
			l <= Oi || a.canvas.draw((t) => {
				let n = e.getPosition(), u = s.getPosition(), d = c.getPosition(), f = e.retina.linksDistance ?? Ai;
				if (I(n, u) > f || I(d, u) > f || I(d, n) > f) return;
				let p = V(this._engine, i.color);
				p ||= et(e, s, r.id === void 0 ? a.particles.linksColor : a.particles.linksColors.get(r.id)), p && Ti({
					context: t,
					pos1: n,
					pos2: u,
					pos3: d,
		...
		}, this._getLinkFrequency = (e, t) => Di([e, t], this._freqs.links), this._getTriangleFrequency = (e, t, n) => Di([
			e,
			t,
			n
	...
	drawParticle(e, t) {
		...
		if (!n?.length) return;
		let i = n.filter((e) => r.links && (r.links.frequency >= Mi || this._getLinkFrequency(t, e.destination) <= r.links.frequency));
		for (let e of i) this._drawTriangles(r, t, e, i), e.opacity > Oi && (t.retina.linksWidth ?? ki) > ki && this._drawLinkLine(t, e);
	}
	async init() {
	...
	getPlugin(e) {
		return Promise.resolve(new Ni(e, this._engine));
	}
	loadOptions() {}
	needsPlugin() {
		return !0;
...
async function Ii(e, t = !0) {
	e.checkVersion("3.9.1"), await Si(e, t), await Fi(e, t);
}
//#endregion
//#region node_modules/@tsparticles/preset-triangles/esm/options.js
var Li = {
...
//#endregion
//#region node_modules/@tsparticles/preset-triangles/esm/index.js
async function Ri(e, t = !0) {
	await ui(e, !1), await Ii(e, !1), await e.addPreset("triangles", Li, !1), await e.refresh(t);
}
//#endregion
